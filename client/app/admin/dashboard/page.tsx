...
export default function AdminDashboard() {
  ...
  const handleResendEmail = async (orderId: string) => {
    ...
    const token = localStorage.getItem("adminToken");
    if (!token) {
        alert("Sesiune expirată. Te rugăm să te re-autentifici.");
        return;
    }
    ...
    return (
      <div className="min-h-screen bg-[#0a0905] flex items-center justify-center text-yellow-500">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="animate-pulse text-sm font-bold tracking-widest uppercase">
            Se încarcă datele...
          </p>
        </div>
      </div>
          ...
          <div className="relative z-10">
            ...
            <p className="text-yellow-100/40 flex items-center gap-4 text-sm font-bold uppercase tracking-wider">
              <span className="flex items-center gap-2">
                <span className="material-symbols-outlined text-base">verified_user</span> Administrator
              </span>
            </p>
          </div>
