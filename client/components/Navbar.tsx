  ...
  return (
    <header className="fixed top-0 w-full z-[70] bg-[#0a0905]/90 backdrop-blur-md border-b border-yellow-900/30 shadow-2xl">
      <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
        ...
        {/* LOGO */}
        <Link href="/" className="flex items-center gap-3 group relative z-[80]">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-yellow-600/80 font-bold tracking-[0.2em] uppercase">Live in Concert</span>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-10">
          {!isAdmin ? (
            // PUBLIC NAVIGATION
            <>
              <Link href="/" className="text-yellow-100/70 hover:text-yellow-500 font-bold text-xs uppercase tracking-[0.15em] transition-colors relative group">
                Eveniment
                <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-yellow-500 transition-all group-hover:w-full"></span>
              </Link>
              <Link href="/booking" className="text-yellow-100/70 hover:text-yellow-500 font-bold text-xs uppercase tracking-[0.15em] transition-colors relative group">
                Bilete
                <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-yellow-500 transition-all group-hover:w-full"></span>
              </Link>
            ...
            <>
              ...
              <Link href="/" className="text-yellow-600 hover:text-red-400 font-bold text-xs uppercase tracking-widest transition-colors flex items-center gap-2">
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 bg-[#0a0905] z-[75] md:hidden flex flex-col pt-32 px-8 animate-in fade-in duration-300">
            ...
            <div className="mt-auto pb-12 relative z-10">
              <p className="text-yellow-700 font-bold uppercase tracking-widest text-xs mb-6 text-center">Urmărește-ne</p>
              <div className="flex justify-center gap-6">
                {['facebook', 'instagram', 'youtube'].map(social => (
                  <a key={social} href="#" className="size-14 rounded-full bg-yellow-900/10 border border-yellow-500/20 flex items-center justify-center text-yellow-500 hover:bg-yellow-500 hover:text-black transition-all duration-300">
                    <span className="material-symbols-outlined text-2xl">{social === 'facebook' ? 'groups' : social === 'instagram' ? 'photo_camera' : 'play_circle'}</span>
                  </a>
                ))}
              </div>
            </div>
